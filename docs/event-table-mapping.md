...
| `event` | `file.edited` | `EventFileEdited` | `file-edited.ts` | `file.edited` |
...
| `deleted_at` | `Date.now()` 转换时生成 | `session.deleted` | `EventSessionDeleted` | `SessionDeletedEvent` |
...
| `first_event_at` | `Date.now()` 转换时生成 | `session.created` | `EventSessionCreated` | `SessionCreatedEvent` |
| `last_event_at` | `Date.now()` 转换时生成 | 全部 | - | - |
| `duration_ms` | 计算值：`last_event_at - first_event_at` | - | - | - |
...
| `lines_added` | `event.properties.info.summary?.diffs.reduce(sum => sum.additions)` | `message.updated` | `EventMessageUpdated` | `MessageUpdatedEvent` |
| `lines_deleted` | `event.properties.info.summary?.diffs.reduce(sum => sum.deletions)` | `message.updated` | `EventMessageUpdated` | `MessageUpdatedEvent` |
| `error_count` | +1 | `session.error` | `EventSessionError` | `SessionErrorEvent` |
| `event_count` | +1 | 全部 | - | - |

---

## 3. projection_daily_model_usage 表

仅处理 `message.updated` 事件，按 `(date, project_path, model)` 聚合。

| 字段 | SDK 事件原始字段 | SDK 事件名称 | SDK 事件 | 映射事件 |
|------|----------------|------------|---------|---------|
| `date` | `Date.now()` → `YYYY-MM-DD` | `message.updated` | `EventMessageUpdated` | `MessageUpdatedEvent` |
...
| `assistant_messages` | `event.properties.info.role === "assistant"` 时 +1 | `message.updated` | `EventMessageUpdated` | `MessageUpdatedEvent` |
| `total_tokens` | `info.tokens.input + info.tokens.output + info.tokens.reasoning + info.tokens.cache.read + info.tokens.cache.write` | `message.updated` | `EventMessageUpdated` | `MessageUpdatedEvent` |
| `input_tokens` | `event.properties.info.tokens.input` | `message.updated` | `EventMessageUpdated` | `MessageUpdatedEvent` |
| `output_tokens` | `event.properties.info.tokens.output` | `message.updated` | `EventMessageUpdated` | `MessageUpdatedEvent` |
| `reasoning_tokens` | `event.properties.info.tokens.reasoning` | `message.updated` | `EventMessageUpdated` | `MessageUpdatedEvent` |
...
| `started_at` | `Date.now()` 转换时生成 | `tool.execute.before` | 钩子参数 | `ToolExecuteBeforeEvent` |
| `completed_at` | `Date.now()` 转换时生成 | `tool.execute.after` | 钩子参数 | `ToolExecuteAfterEvent` |
| `completed_at` | `Date.now()` 转换时生成 | `tool.failed` | `EventMessagePartUpdated` | `ToolFailedEvent` |
